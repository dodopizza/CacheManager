...
namespace CacheManager.Core
{
    public partial class BaseCacheManager<TCacheValue>
    {
#if !NET40
        /// <inheritdoc />
        public ValueTask ExpireAsync(string key, ExpirationMode mode, TimeSpan timeout)
            => ExpireInternalAsync(key, null, mode, timeout);

        /// <inheritdoc />
        public ValueTask ExpireAsync(string key, string region, ExpirationMode mode, TimeSpan timeout)
            => ExpireInternalAsync(key, region, mode, timeout);

        private async ValueTask ExpireInternalAsync(string key, string region, ExpirationMode mode, TimeSpan timeout)
        {
            CheckDisposed();
        ...
        /// <inheritdoc />
        public ValueTask ExpireAsync(string key, DateTimeOffset absoluteExpiration)
        {
        ...
        /// <inheritdoc />
        public ValueTask ExpireAsync(string key, string region, DateTimeOffset absoluteExpiration)
        {
            var timeout = absoluteExpiration.UtcDateTime - DateTime.UtcNow;
            if (timeout <= TimeSpan.Zero)
            {
        ...
        /// <inheritdoc />
        public ValueTask ExpireAsync(string key, TimeSpan slidingExpiration)
        {
        ...
        /// <inheritdoc />
        public ValueTask ExpireAsync(string key, string region, TimeSpan slidingExpiration)
        {
            if (slidingExpiration <= TimeSpan.Zero)
            {
                throw new ArgumentException("Expiration value must be greater than zero.", nameof(slidingExpiration));
        ...
        /// <inheritdoc />
        public ValueTask RemoveExpirationAsync(string key)
        {
            return ExpireAsync(key, ExpirationMode.None, default(TimeSpan));
        }

        /// <inheritdoc />
        public ValueTask RemoveExpirationAsync(string key, string region)
        {
